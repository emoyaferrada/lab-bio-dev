...
class facturasModel
{
	...
	public function guardar_factura($pagada,$num_cuotas,$fecha,$cliente,$monto,$num_factura,$foma_pago,$obs,$valor_cuota)/// guardar factura de pago
	{
		try 
		{
			$sql = "INSERT INTO factura(fecha, numero, pagada, fecha_factura, monto, cliente_id, numero_cuota, forma_pago, observacion, valor_cuota)
					VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$this->pdo->prepare($sql)
				 ->execute(array(date("Y/m/d"), $num_factura, $pagada, $fecha, $monto, $cliente, $num_cuotas, $foma_pago, $obs, $valor_cuota));		
	...
	public function listar_deudas($rut)
	{
		try
		{
			...
			$stm = $this->pdo->prepare("SELECT f.id as id, TO_CHAR(f.fecha, 'dd/mm/yyyy') as fecha, TO_CHAR(f.fecha_factura, 'dd/mm/yyyy') as fecha_factura, c.nombre as nombre_cliente, f.monto, f.numero_cuota, f.valor_cuota, f.numero
				FROM factura AS f, cliente AS c WHERE c.rut=f.cliente_id AND c.rut='".$rut."' ORDER BY f.fecha DESC;");
			$stm->execute();
			foreach($stm->fetchAll(PDO::FETCH_OBJ) as $r)
			{
				$monto_pagado="";
