...
class facturasModel
{
	...
	public function guardar_factura($pagada,$num_cuotas,$fecha,$cliente,$monto,$num_factura,$forma_pago,$obs,$valor_cuota,$ges,$num_orden,$fecha_orden, $valor_orden,$archivo_orden,$tipo_pago)/// guardar factura de pago
	{
		try 
		{
			$sql = "INSERT INTO factura(fecha, numero, pagada, fecha_factura, monto, numero_cuota, forma_pago, observacion, valor_cuota, cliente_id, oc_ges, oc_numero, oc_fecha, oc_valor, oc_archivo, tipo_pago) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$this->pdo->prepare($sql)->execute(array(date("Y/m/d"), $num_factura, $pagada, $fecha, $monto, $num_cuotas, $forma_pago, $obs, $valor_cuota, $cliente, $ges, $num_orden, $fecha_orden, $valor_orden, $archivo_orden, $tipo_pago));		
	...
	public function guardar_pago_cuota($id_fact,$fecha_pago,$numero_factura,$fecha_factura,$monto_pagado,$forma_pago,$num_cuotas,$pagada,$tipo_pago) /// pago de cuota 
	{
		try 
		{		
			$sql = "INSERT INTO pago_cuotas(cantidad_cuotas, fecha_pago, numero_factura, fecha_factura, monto_pagado, factura_id, forma_pago, tipo_pago) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
			$this->pdo->prepare($sql)->execute(array($num_cuotas, $fecha_pago, $numero_factura, $fecha_factura, $monto_pagado, $id_fact, $forma_pago, $tipo_pago));
	...
	public function listar_deudas($rut)
	{
		try
		{
			...
			$result = array();
			$actual=date("Y-m-d");
			$stm = $this->pdo->prepare("SELECT f.id as id, TO_CHAR(f.fecha, 'dd/mm/yyyy') as fecha, TO_CHAR(f.fecha_factura, 'dd/mm/yyyy') as fecha_factura, c.nombre as nombre_cliente, f.monto, f.numero_cuota, f.valor_cuota, f.numero, f.tipo_pago
			...
			foreach($stm->fetchAll(PDO::FETCH_OBJ) as $r)
			{
				...
				foreach($stm2->fetchAll(PDO::FETCH_OBJ) as $r2)
				{
					...
					$cuotas_pendiente = $r->numero_cuota - $cuotas_pagadas;
					$monto_pendiente = $r->monto - $monto_pagado;
				}
				
				$i++;
				if($r->tipo_pago=="cuota"){
				...
				$tabla_contenido.='<tr>
					...
					<td>'.$cuotas_pendiente.'<input type="hidden" id="cuotas_pagadas'.$i.'" name="cuotas_pagadas'.$i.'" value="'.$cuotas_pagadas.'"></td>
					<td>'.$valor_cuota.'<input type="hidden" id="valor_cuota'.$i.'" name="valor_cuota'.$i.'" value="'.$r->valor_cuota.'"></td>
					<td>'.$r->monto.'<input type="hidden" name="monto_total'.$i.'" value="'.$r->monto.'"></td>
					<td>'.$monto_pagado.'<input type="hidden" name="monto_pagado'.$i.'" value="'.$monto_pagado.'"></td>
			...
			$tabla = '<table class="table">
					<thead>
					  ...
					  <th>Tipo Pago</th>
					  <th>Total Cuotas</th>
					  <th>Cuotas Pedientes</th>
					  <th>Valor Cuotas</th>					  
					  <th>Monto Total</th>
					  <th>Monto Pagado</th>
					  <th>Monto Pendiente<input type="hidden" id="cont_fact" name="cont_fact" value="'.$i.'"></th>					  
					</thead>
					<tbody>			
					  '.$tabla_contenido.'					  
					</tbody>
	...
	public function facturas_pendientes()
	{
		...
		{
			$salida="";
			$sql="SELECT f.id, f.numero, f.monto, f.fecha, f.numero_cuota, f.tipo_pago, c.rut, c.nombre FROM Cliente AS c, factura AS f WHERE c.rut=f.cliente_id AND f.pagada='false' ORDER BY c.nombre;";
			$stm = $this->pdo->prepare($sql); 
			$stm->execute();
			foreach ($stm->fetchAll(PDO::FETCH_OBJ) as $r) {
				...
				}else if($r->tipo_pago=='abono'){
					$stm2 = $this->pdo->prepare("SELECT SUM(monto_pagado) AS monto_pagado FROM pago_cuotas WHERE factura_id=".$r->id);
					$stm2->execute();
					foreach($stm2->fetchAll(PDO::FETCH_OBJ) as $r2)
					{
						$monto_pagado=$r2->monto_pagado;
				...
				$salida.="<tr><td>".$r->numero."</td><td>".$r->nombre."</td><td>".$r->fecha."</td><td>$ ".$r->monto."</td><td>$ ".$monto_pagado."</td><td>".$r->numero_cuota."</td><td>".$cuotas_pagadas."</td><td><div class='progress' style='height: 20px;'><div class='progress-bar ".$color."' role='progressbar' aria-valuenow='".$porc."' aria-valuemin='0' aria-valuemax='100' style='width: ".$porc."%;'>".$porc."%</div></div></td></tr>";
			}
			if($salida==""){
	...
	function listar_facturas($rut,$fechai,$fechaf){
		try
		{
			...
			$i = 0;			
			$stm = $this->pdo->prepare("SELECT f.id as id, TO_CHAR(f.fecha, 'dd/mm/yyyy') as fecha, TO_CHAR(f.fecha_factura, 'dd/mm/yyyy') as fecha_factura, c.nombre as nombre_cliente, f.monto, f.numero_cuota, f.valor_cuota, f.numero, f.pagada, f.tipo_pago
	...
	function ver_detalle($id){
		try
		{
			$stm = $this->pdo->prepare("SELECT f.id as id, TO_CHAR(f.fecha, 'dd/mm/yyyy') as fecha, TO_CHAR(f.fecha_factura, 'dd/mm/yyyy') as fecha_factura, c.nombre, f.monto, f.numero_cuota, f.valor_cuota, f.tipo_pago, f.numero, f.forma_pago, f.observacion, f.pagada
