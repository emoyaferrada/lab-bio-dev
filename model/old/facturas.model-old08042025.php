...
class facturasModel
{
	...
	public function guardar_factura($pagada,$monto,$cliente,$ges,$num_orden,$fecha_orden,$valor_orden,$archivo_orden,$tipo_pago,$valor_factura,$archivo_factura,$fecha_factura,$num_factura,$obs)/// guardar factura de pago
	{
		try 
		{
			$sql = "INSERT INTO factura(fecha, pagada, monto, cliente_id, oc_ges, oc_numero, oc_fecha, oc_valor, oc_archivo, tipo_pago, factura_valor, factura_archivo, factura_fecha, factura_numero, observacion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$this->pdo->prepare($sql)->execute(array(date("Y/m/d"), $pagada, $monto, $cliente, $ges, $forma_pago, $num_orden, $fecha_orden, $valor_orden, $archivo_orden, $tipo_pago, $fecha, $num_factura, $obs));		
	...
	public function listar_deudas($rut,$fact)
	{
		try
		{
			...
			$result = array();
			$actual=date("Y-m-d");
			$stm = $this->pdo->prepare("SELECT f.id as id, TO_CHAR(f.fecha, 'dd/mm/yyyy') as fecha, TO_CHAR(f.factura_fecha, 'dd/mm/yyyy') as fecha_factura, c.nombre as nombre_cliente, f.monto, f.factura_numero
	...
	public function facturas_pendientes()
	{
		...
		{
			$salida="";
			$sql="SELECT f.id, f.factura_numero, f.factura_valor, f.monto, f.fecha c.rut, c.nombre FROM Cliente AS c, factura AS f WHERE c.rut=f.cliente_id AND f.pagada='false' ORDER BY c.nombre;";
			$stm = $this->pdo->prepare($sql); 
			$stm->execute();
			foreach ($stm->fetchAll(PDO::FETCH_OBJ) as $r) {
	...
	function listar_facturas($rut,$fechai,$fechaf){
		try
		{
			...
			$i = 0;			
			$stm = $this->pdo->prepare("SELECT f.id as id, TO_CHAR(f.fecha, 'dd/mm/yyyy') as fecha, TO_CHAR(f.factura_fecha, 'dd/mm/yyyy') as fecha_factura, c.nombre as nombre_cliente, f.monto, f.numero_cuota, f.valor_cuota, f.factura_numero, f.factura_valor, f.pagada, f.tipo_pago
