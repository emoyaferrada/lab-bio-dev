...
class comprasModel
{
	...
	public function guardar_insumo($nombre, $descrip, $unidad, $alerta, $tiempo, $fecha, $certificado)/// guardar insumos
	{
		try 
		{
			$sql = "INSERT INTO insumo(nombre, descripcion, unidad_medida, sub_tipo, stock, stock_alerta, fecha_ingreso, fecha_vencimiento, estado, fecha_baja, motivo_baja, tiempo_compra, certificado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$this->pdo->prepare($sql)->execute(array($nombre, $descrip, $unidad, '', '', $alerta, $fecha, '2000-01-01', '', '2000-01-01', '', $tiempo, $certificado));		
	...
	public function guardar_proveedor($nombre, $rut, $direccion, $fono, $correo, $contacto)/// guardar analisis segun factura
	{
		try 
		{
			$sql = "INSERT INTO proveedores(nombre, rut, direccion, fono, correo, contacto) VALUES (?, ?, ?, ?, ?, ?)";
			$this->pdo->prepare($sql)->execute(array($nombre, $rut, $direccion, $fono, $correo, $contacto));
			return 1;			
		}		
		catch (Exception $e) 
		{
	...
	public function option_unidades()
	{
		try
		{
			...
			$stm = $this->pdo->prepare("SELECT id, nombre FROM unidad_medida ORDER BY nombre DESC;");
			$stm->execute();
	...
	public function guardar_pago($id_fact,$fecha_pago,$monto_pagado,$forma_pago,$pagada,$tipo_pago,$comprobante) /// pago				
	{
		try 
		{		
			$sql = "INSERT INTO pagos(fecha_pago, monto_pagado, factura_id, forma_pago, tipo_pago, comprobante) VALUES (?, ?, ?, ?, ?, ?)";
			$this->pdo->prepare($sql)->execute(array($fecha_pago, $monto_pagado, $id_fact, $forma_pago, $tipo_pago, $comprobante));
		...
		try
		{
			...
			foreach($stm->fetchAll(PDO::FETCH_OBJ) as $r)
			{
				...
				$tabla_contenido.='<tr>	
						...
						<td>'.$r->descripcion.'</td>
						<td><input type="text" class="col-4 form-control" onchange="sumar('.$i.');" name="valor'.$i.'" id="valor'.$i.'" value="'.$r->valor.'"></td>             
					  </tr> ';				
			}
	...
	public function listar_deudas($rut,$fact)
	{
		try
		{
			...
			$result = array();
			$actual=date("Y-m-d");
			$stm = $this->pdo->prepare("SELECT f.id as id, TO_CHAR(f.fecha, 'dd/mm/yyyy') as fecha, TO_CHAR(f.fecha_factura, 'dd/mm/yyyy') as fecha_factura, c.nombre as nombre_cliente, f.monto, f.numero
				FROM factura AS f, cliente AS c WHERE c.rut=f.cliente_id AND c.rut='".$rut."'".$opcion." AND f.pagada='0' ORDER BY f.fecha DESC;");
			$stm->execute();
			foreach($stm->fetchAll(PDO::FETCH_OBJ) as $r)
			{
				$monto_pagado="";
				...
				$cuotas_pendiente=0;
				$monto_pendiente = 0;
				$stm2 = $this->pdo->prepare("SELECT SUM(monto_pagado) AS monto_pagado FROM pagos WHERE factura_id=".$r->id);
				$stm2->execute();
	...
	public function facturas_pendientes()
	{
		...
		{
			$salida="";
			$sql="SELECT f.id, f.numero, f.monto, f.fecha c.rut, c.nombre FROM Cliente AS c, factura AS f WHERE c.rut=f.cliente_id AND f.pagada='false' ORDER BY c.nombre;";
			$stm = $this->pdo->prepare($sql); 
			$stm->execute();
			foreach ($stm->fetchAll(PDO::FETCH_OBJ) as $r) {
