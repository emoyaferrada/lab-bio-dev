...
?>
  <!--form  method="post" action="../view/analisis/nuevo-batch.php">
    <div class="form-group row">
      <div class="col">
        <select id="tipo_analisis" name="tipo_analisis" class="custom-select">
          <option value=0>Seleccione tipo de análisis</option>
          <? //echo $analisis->tipo_analisis_select();?>
        </select>
      </div>
      <div>
        <button type="submit" id="mostrar" name="mostrar" value="1" class="btn btn-primary">Mostrar</button>
      </div>
    </div>
  </form-->
<?
  ...
  //if ($_POST){
    ...
    echo '<form  method="post" action="../../controller/analisis.controller.php">
