...
?>
  ...
  <form method="post" name="frmfacturas" id="frmfacturas" onsubmit="validar()" action="../controller/facturas.controller.php" enctype="multipart/form-data">
	...
	<div class="form-group row"> 
      ...
      <div class="col">
        <label for="Valor Abono"><b>Valor Abono/Cuota</b></label> 
	...
	<div class="form-group row">
      <div class="offset-4 col-8">
        <button name="guardar_factura" id="guardar_factura" value="1" type="submit" class="btn btn-primary">Guardar</button>
      </div>    
    </div>
  </form>
</div>
    ...
    $( document ).ready(function() {	
		...
		$('#tipo_pago').on('change', function(){
			...
			if(tipo_pago == "cuota"){
				...
				$("#valor_cuota").prop('disabled', true);
				$("#valor_cuota").val('');		
				$("#info_cuota").html('');
		...
		$('#valor_cuota').on('change', function(){
			...
			if(tipo_pago == "abono"){
				...
				alert ("Debe cambiar a 'ABONO' en Tipo de Pago");
			}			
		})
...
function validar(){
	...
	if(cliente==0){		
		...
		return false;		 	 
	 }else if(tipo_pago=="abono" && ($("#valor_cuota").val()=="" || $("#valor_cuota").val()==0)){
		alert("DEBE INGRESAR NUMERO DE CUOTAS");
		return false;		 
	 }else{
		 //$("#frmfacturas").submit();
		 //document.frmfacturas.submit();
		 //document.getElementById('frmfacturas').submit()
		 return true;
	 }
